...
{
    public class ImageAIDefaultCommunicator : IImageAICommunicator
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly string _url;

        public ImageAIDefaultCommunicator()
        {
            _url = "http://localhost:21119/query";
        ...
        public async Task<string> GetImageUrlAsync(string query)
        {
            try
            {
                ...
                return responseDictionary["img_url"];
            }
            catch (Exception ex)
            {
                return ex.Message;
